...
class Player extends Component {
  ...
  goBackward = () => {
    const { currentTime, songIndex } = this.state;
    if(currentTime < 3 && songIndex !== 0 ){
      this.setState({
  ...
  goForward = () => {
    const { shuffle, songIndex } = this.state;
    this.setState({
      songIndex: shuffle ? this.randomSongIndex() : songIndex + 1,
      currentTime: 0,
    });
    this.audio.seek(0);
  }
  ...
  render() {
    ...
    if( !this.state.shuffle && this.state.songIndex + 1 === this.props.songs.length ){
      ...
      forwardButton = <Icon onPress={ this.goForward } style={ styles.forward } name="ios-skipforward" size={25} color="#fff" />;
    }

    let volumeButton;
    if( this.state.muted ){
      volumeButton = <Icon onPress={ this.toggleVolume } style={ styles.volume } name="android-volume-off" size={18} color="#fff" />;
    } else {
      volumeButton = <Icon onPress={ this.toggleVolume } style={ styles.volume } name="android-volume-up" size={18} color="#fff" />;
    }

    let shuffleButton;
    if( this.state.shuffle ){
      shuffleButton = <Icon onPress={ this.toggleShuffle } style={ styles.shuffle } name="ios-shuffle-strong" size={18} color="#f62976" />;
    } else {
      shuffleButton = <Icon onPress={ this.toggleShuffle } style={ styles.shuffle } name="ios-shuffle-strong" size={18} color="#fff" />;
    }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={ styles.headerClose }>
          <Icon onPress={() => {}} name="chevron-down" size={15} color="#fff" />
        </View>
        <Image
        ...
        <View style={ styles.controls }>
          { shuffleButton }
          <Icon onPress={ this.goBackward } style={ styles.back } name="ios-skipbackward" size={25} color="#fff" />
          { playButton }
          { forwardButton }
          { volumeButton }
        </View>
